...
public class CommunicationChannel {
    ...
    public void checkCompleted() {
        ...
        for (Frame temp : tcpCommList) {
            ...
            } else if (DataTypeHelper.getTcpPortFlags(temp).contains("SYN") && foundSyn.equals(1)) {
                if (DataTypeHelper.getTcpPortFlags(temp).contains("SYN") && srcPsyn.equals(temp.getIpv4parser().getTcpParser().getDestinationPort())) {
                    open = 1;
                }
            } else if (DataTypeHelper.getTcpPortFlags(temp).contains("FIN") && foundFin.equals(0)) {
                foundFin = 1;
                if (srcPfin == 0) {
                    srcPfin = temp.getIpv4parser().getTcpParser().getSourcePort();
                }

            } else if (DataTypeHelper.getTcpPortFlags(temp).contains("FIN") && foundFin.equals(1)) {
